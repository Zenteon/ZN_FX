...
uniform bool BLOCK_SCATTER <
	...
	ui_tooltip = "Samples normals while casting to prevent self illumination || Very Lerge Performance Impact";
	ui_category = "Sampling";
> = 0;
...
uniform int PREPRO_SETTINGS <
	...
	ui_text = "Preprocessor Definition Guide:\n"
			"\n"
			"ZNRY_MAX_LODS - The maximum LOD sampled, has a direct performance impact, and an exponential impact on ray range\n"
			"\n"
			"ZNRY_RENDER_SCL - The resolution scale for GI, default is automatically selected based on resolution\n"
			"\n"
...
float3 DAMPGI(float2 xy, float2 offset)
{
    ...
    dir[0] = normalize( float2(0.0, 1.0) - offset.xy);
    dir[1] = normalize( float2(0.7071, 0.7071) - offset.xy);
    dir[2] = normalize( float2(1.0, 0.0) - offset.xy);
    dir[3] = normalize( float2(0.7071, -0.7071) - offset.xy);
    dir[4] = normalize( float2(0.0, -1.0) - offset.xy);
    dir[5] = normalize( float2(-0.7071, -0.7071) - offset.xy);
    dir[6] = normalize( float2(-1.0, 0.0) - offset.xy);
    dir[7] = normalize( float2(-0.7071, 0.7071) - offset.xy);
    
    float trueD = ReShade::GetLinearizedDepth(xy);
    float3 surfN = normalize(1.0 + 2.0 * tex2D(NorSam, xy).rgb);
    float d = trueD;
    ...
    for(int i = 0; i < 8; i++){
 		...
 		for(int ii = 2; ii <= LODS; ii++)
    	{
    		...
    		rp.xy += abs(offset.r) * 5.0 * (dir[i] * pow(2, ii)) / res;
    		...
    		float sBias = SHADOW_BIAS;//0.00000000001;
   		 int sh;
   		 int sh2;
   		 if(SHADOW == 0) {sh = 1;}
			...
			if(d < (trueD + shd * shvMin.z) + sBias || sh == 1)
			{
				float3 col = tex2Dlod(LumSam, float4(rp.xy, 0, iLOD)).rgb;
				float smb = 1.0;
				if(BLOCK_SCATTER == 1)
				{
					float3 nor = -1.0 + 2.0 * tex2Dlod(NorSam, float4(rp.xy, 0, iLOD)).rgb;
					smb = 0.5 + 0.5 * dot(surfN, -nor);
					smb *= 2.0;
				}
				
				float ed = 1.0 + pow(distance(texXY, 0.0) / f, 2.0);
				float pd = 1.0 + 1.0 * distance(rp.xy, xy);
				float cd = 1.0 + (pow(distance(eyeXY, texXY) / f, 2.0));
				float amb = 0.5 + 0.5 * dot(surfN, normalize(rp - float3(xy, trueD)));
				
				col *= ed;
				l += pow(2.0, iLOD) * smb * amb * (col / (ed * cd));
			}
...
float4 RawGI(float4 vpos : SV_Position, float2 texcoord : TexCoord) : SV_Target
{
	...
	return float4(DAMPGI(texcoord, 1.0 - 1.0 * noise.xy), 1.0);
}
